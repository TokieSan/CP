...
vi deg;
vi a;
void build(vector<int>::iterator s, vector<int>::iterator e, int c){
    if(s>e || s<a.begin() || e>a.end()||s==e) return;

    auto m = max_element(s,e);
    deg[m-a.begin()]=c;
    build(s,m,c+1);
    build(m+1,e,c+1);
}
...
void solve() {
   ...
   a.resize(n);
    ...
    build(a.begin(),a.end(),0);
      // must start and end to the number iteself
    FOR(i, deg.begin(), deg.end()){
        cout << *i << ' ';
    }
    cout << '\n';
   // clear();
       //يلعن كسم دي مسالة سهله فشخ بس مش لاقي كسم الout of bounds دا
